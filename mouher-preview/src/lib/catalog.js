...
export async function loadCatalog(config = medusaConfig) {
  ...
  for (const dynamicSource of dynamicSources) {
    try {
      const response = await dynamicSource.fetcher(config);
      ...
      console.error(`Failed to load catalog from ${dynamicSource.label}:`, error);
    }
  }
  ...
  if (config.allowStaticCatalogFallback) {
    return loadStaticFallbackCatalog(
      dynamicSources.length
        ? "Dynamic catalog API is not reachable, so development fallback products are shown."
        : "Dynamic catalog API is not configured, so development fallback products are shown."
    );
  }

  return {
    ...EMPTY_CATALOG,
    notice: dynamicSources.length
      ? "Dynamic catalog API is not reachable."
      : "Connect Mouher API or Medusa Store API settings to load products.",
  }
}
