...
export default function App() {
  ...
  return <div className="app-shell"><Navigation activePage={page} onPageChange={setPage} onLogout={() => { endOwnerSession(); setAuthenticated(false); setWorkspace(null); }} /><main className="main-content"><header className="topbar"><div><span className="eyebrow">OWNER DASHBOARD / {page.toUpperCase()}</span><h1>{page === "overview" ? "Business overview" : `${page[0].toUpperCase()}${page.slice(1)} register`}</h1></div><div className="top-controls"><label>Report range <select value={days} onChange={changeRange}><option value="7">7 days</option><option value="30">30 days</option><option value="90">90 days</option></select></label><span className="owner-badge">pmembari</span></div></header>{workspace?.source === "local-preview" && <div className="notice-message">Owner API is not configured. Set VITE_MOUHER_API_URL for live Django reports.</div>}{error && <div className="error-message">{error}</div>}{loading && <div className="loading-message">Refreshing owner data...</div>}{page === "overview" ? <DashboardHome analytics={analytics} products={products} orders={orders} customers={customers} inventory={inventory} /> : <ResourcePage page={page} rows={rows} analytics={analytics} />}</main></div>;
}
